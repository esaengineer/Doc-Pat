...
if(isset($_POST["action"]))
{
	if($_POST["action"] == 'fetch')
	{
		...
		foreach($result as $row)
		{
			...
			{
				$status = '<button type="button" name="status_button" class="btn btn-danger button-18 btn-sm status_button" data-id="'.$row["doctor_schedule_id"].'" data-status="'.$row["doctor_schedule_status"].'">Inactive</button>';
			}
